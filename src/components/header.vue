      ...
      <Flex v-if="!navOnly" grow justify="end" align="center">
        ...
        <form class="fk-header__form" @submit.prevent="login">
          <div class="fk-header__form__field">
            <Input v-model="email" name="email" type="email" size="small" :wrong="wrongInput"
                   :placeholder="formItems.emailPlaceholder" :aria-label="formItems.emailPlaceholder"
                   pid="header.email"></Input>
            <a :href="formItems.registerUrl" :aria-label="formItems.registerLabel">{{
              formItems.registerLabel }}</a>
          </div>
          <div class="fk-header__form__field">
            <Input v-model="password" name="password" type="password" size="small" :wrong="wrongInput"
                   :placeholder="formItems.passwordPlaceholder" :aria-label="formItems.passwordPlaceholder"
                   pid="header.password"></Input>
            <a :href="formItems.retriveUrl" :aria-label="formItems.retriveLabel">{{ formItems.retriveLabel
              }}</a>
          </div>
          <Button type="submit" color="primary" size="small" class="fk-header__form__login"
                  pid="header.loginbutton">
            {{ formItems.submitText }}
          </Button>
        </form>
        <!-- login button -->
        <Button link color="primary" size="small" class="fk-header__btn-login" :href="registrationLink"
                :aria-label="formItems.submitText">
          {{ formItems.submitText }}
        </Button>
        <!-- shop button -->
        <Button link color="secondary" size="small" class="fk-header__btn-shop" :href="formItems.shopLink"
                :aria-label="formItems.shopText">
          {{ formItems.shopText }}
        </Button>
        <!-- cart button -->
        <Button link color="secondary" size="small" class="fk-header__btn-cart" :href="formItems.cartLink"
                aria-label="cart">
